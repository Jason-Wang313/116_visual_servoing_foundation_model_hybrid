# Paper 116 ICLR Submission-Readiness Execution Plan

Paper: `visual_servoing_foundation_model_hybrid`

Date: 2026-06-15

Starting commit: `f764b9ee3809256620bc298f461a77de2c0f3dcd`

Current terminal state entering continuation: `STRONG_REVISE`, not ICLR-main ready.

## Goal

Rebuild Paper 116 into the strongest honest submission package supported by the available local evidence, without overstating readiness. The continuation pass must preserve the terminal decision if the evidence remains local-only, and must not claim ICLR-main readiness without real robot or independent high-fidelity validation.
...
- Expand failure documentation from 3 cases to at least 8 concrete visual-servo/foundation-policy boundary cases.
...
- Copy only `paper/main.pdf` to `C:/Users/wangz/Downloads/116.pdf`.
...
- Add visual-servo/foundation-policy failure cases covering hidden Jacobian singularities, field-of-view loss, semantic action correctness with local instability, latency-induced override harm, occlusion/depth compounding, uncertainty over-rejection, calibration failure, and oracle headroom.
...
- Verify `C:/Users/wangz/Downloads/116.pdf` SHA256 and size.
- Verify `C:/Users/wangz/Desktop/116.pdf` is absent.
- Scan LaTeX and BibTeX logs for actionable warnings/errors.
- Commit and push the child repo.
- Verify `Jason-Wang313/116_visual_servoing_foundation_model_hybrid` is public and `origin/main` matches local `HEAD`.

## Factory Ledger Update

After Paper 116 passes the continuation audit, update:
...
The ledgers must state that Papers 61-116 have passed continuation re-audit and Paper 117 is next.
